 ...
 * @property float $deliverycost Стоимость доставки
...
class Order extends OrmObject
{
    ...
    function _init()
    {
        ...
        $properties
            ...
            ->append(array(
                'reg_fio' => new Type\Varchar(array(
                    'description' => t('Ф.И.О'),
                    'meVisible' => false,
                )),
                ...
                'reg_phone' => new UsersCustomType\VerifiedPhone(array(
        ...
        $properties
            ...
            ->append(array(
                ...
                'addr_address' => new Type\Varchar(array(
                    ...
                    {
                        $config = ConfigLoader::byModule($object);
                        if ($config['require_address'] && empty($value) && (!$object['delivery'] || !$object->getDelivery()->getTypeObject()->isMyselfDelivery())) {
                            return $error;
                        }
                        return true;
    ...
    function afterWrite($flag)
    {
        ...
        EventManager::fire('order.change', array('order_before' => $this->this_before_write, 'order' => $this));
    }
    ...
    */
    ...
    function canOnlinePay(): bool
    {
        ...
        if (!in_array($this['status'], UserStatusApi::getStatusesIdByType(UserStatus::STATUS_WAITFORPAY))) {
            return false;
        }
        if ($this->is_payed) {
            return false;
    ...
    */
    ...
    function setAddressFromSelectedAddress()
    {
        ...
        if (!empty($region['id'])) {
            ...
            $this['addr_region'] = $address['region'] = $region['id'];
        }
        if (!empty($country['id'])) {
            $this['addr_country_id'] = $country['id'];
            $this['addr_country'] = $address['country'] = $country['id'];
        }
