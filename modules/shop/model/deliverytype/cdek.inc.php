...
class Cdek extends AbstractType implements interfaceIonicMobile
{
    ...
    protected $tariffId = array();  //Идентификатор тарифа по которому будет произведена доставка
    protected $delivery_cost_info = array(); //Стоимость доставки по данному расчётному классу
    protected $cache_pochtomates = array(); // Кэшированный список ПВЗ
    protected $cache_city_id = array(); // Кэшированные id городов
    protected $api;

    public $log;


    protected static $cache_api_requests = array();
    ...
    function getFormObject()
    {
        $properties = new \RS\Orm\PropertyIterator(array(
            ...
            'unit_weight' => new Type\Integer(array(
            ...
            'contact_person_to_print_document' => new Type\Integer(array(
        ...
        if ($response){
            ...
            if (isset($xml->Pvz)){
                foreach($xml->Pvz as $item){
                    if ($city_id || (!$city_id && in_array($address['city'], explode(', ', (string)$item['City'])) )) {
                        $pvz_list[] = $item;
                    }
                }
            }
...
//            $query_info['tariffId'] = $tariffsList[0];  // id тарифа, не нужен, если есть tariffsList
        ...
        foreach ($items as $key=>$item){
            ...
            //Длинна
            $length = $product->getDimensionsObject()->getLength();
            if ($length) {
            ...
            //Ширина
            $width = $product->getDimensionsObject()->getWidth();
            if ($width) {
                $product_arr['width'] = $width;
            } else {
                $this->addError(t('У товара с артикулом %0 не указана ширина.', array($product['barcode'])));
            }

            //Высота
            $height = $product->getDimensionsObject()->getHeight();
            if ($height) {
                $product_arr['height'] = $height;
            } else {
                $this->addError(t('У товара с артикулом %0 не указана высота.', array($product['barcode'])));
            }
            ...
            if ($this->getOption('unit_weight', false)) {
                ...
                if ($unit_weight > $product_arr['weight']) {
                    $this->addError(t('Габариты товара с артикулом %0 слишком велики для данной доставки', array($product['barcode'])));
                }
            }
     ...
     * @return string|false
     */
    public function getCityIdByName(\Shop\Model\Orm\Address $address)
    {
