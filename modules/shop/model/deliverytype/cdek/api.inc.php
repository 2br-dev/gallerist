...
class Api
{
     ...
     * @return mixed
     */
    protected function apiRequest($script, $params = array(), $method = "POST", $use_cache = true)
    {
        $source_id = ($script == 'pvzlist/v1/xml') ? 'delivery_cdek_api_pvz' : 'delivery_cdek_api';
        if (!empty($params)) { //Если параметры переданы
            ksort($params);
        }

        $external_response = (new ExternalRequest($source_id, $this->getApiHost() . $script))
            ->setMethod($method)
            ->setParams($params)
