        ...
        try {
            ...
            foreach ($order->getCart()->getProductItems() as $item) {
                ...
                for ($i = 0; $i < $cart_item['amount']; $i++) {
                    $params['packages'][] = [
                        'weight' => ceil($product->getWeight($cart_item['offer'], ProductApi::WEIGHT_UNIT_G)),
                        'length' => ceil($dimensions->getLength(ProductDimensions::DIMENSION_UNIT_SM)),
                        'height' => ceil($dimensions->getHeight(ProductDimensions::DIMENSION_UNIT_SM)),
                        'width' => ceil($dimensions->getWidth(ProductDimensions::DIMENSION_UNIT_SM)),
                    ];
                }
            }
