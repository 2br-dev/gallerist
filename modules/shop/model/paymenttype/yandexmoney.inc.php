    ...
    */
    protected function getParamsForFZ54Check($transaction)
    {
        ...
        if (!empty($user['phone']) && !$this->getOption('only_email')) {
            $phone = preg_replace(array('/[^\+\d]/'), array(''), $user['phone']);
            if ($this->getOption('fix_rus_phone')) { // если нужно, "исправляем" номер телефона
                $phone = preg_replace(array('/(^9.{9}$)/', '/^8/'), array('+7\1', '+7'), $user['phone']);
            }
        ...
        if ($transaction['order_id']) {
            ...
            if ($cart) {
                ...
                foreach ($delivery as $delivery_item) {
                    ...
                    //ФФД 1.05-1.1
                    $item['paymentMethodType'] = $delivery['payment_method'] ? $delivery['payment_method'] : $order->getDefaultPaymentMethod();
