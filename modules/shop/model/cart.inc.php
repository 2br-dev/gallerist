...
class Cart
{
     ...
     * @throws RSException
     */
    function addProduct($product_id, $amount = 0, $offer = null, $multioffers = array(), $concomitants = array(), $concomitants_amount = array(), $additional_uniq = null, $source = null, $manual = false)
    {
        $api = new CatalogApi();
        /** @var Product $product */
    ...
    protected function addProductData($result, $use_currency)
    {
        ...
        foreach($in_basket as $n => $item) {
            ...
            if ($product->getMaxOrderQuantity() > 0 && $amount > $product->getMaxOrderQuantity()) {
                $result['items'][$n]['amount_error'] = t('Максимальное количество для заказа товара %0', array($product->getMaxOrderQuantity().' '.$product->getUnit()->stitle));
                $result['has_error'] = true;
            }
            ...
            elseif ($module_config['check_quantity'] && $amount > $product->getNum($item[self::CART_ITEM_KEY]['offer'])) {
                $result['items'][$n]['amount_error'] = t('На складе нет требуемого количества товара. В наличии: %0', array((float)$product->getNum($item[self::CART_ITEM_KEY]['offer']).' '.$product->getUnit()->stitle));
                $result['has_error'] = true;
            }
