...
class Checkout extends Front
{
    ...
    public function actionAddress()
    {
        ...
        if ($this->url->isPost()) { //POST
            ...
            if ($this->order['only_pickup_points']) { //Если только самовывоз то исключим поля
                $work_fields = array_diff($work_fields, array('addr_country_id', 'addr_region', 'addr_region_id', 'addr_city', 'addr_zipcode', 'addr_address', 'addr_extra', 'use_addr'));
            ...
            if (!$logout && !$login) {
                ...
                if (in_array($this->order['user_type'], array('person', 'company'))) {
                    ...
                    $allow_fields = array('reg_name', 'reg_surname', 'reg_midname', 'reg_fio',
    ...
    protected function fireStatistic($type)
    {
        if (!isset($this->order['cache_statistic'.$type])) {
            EventManager::fire('statistic', array('type' => $type));
