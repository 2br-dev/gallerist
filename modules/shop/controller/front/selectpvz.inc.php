...
class SelectPvz extends Front
{
    ...
    public function actionIndex()
    {
        ...
        foreach ($delivery_list as $delivery) {
            $pvz_list[$delivery['id']] = $delivery->getTypeObject()->getPvzByAddress($address);
        }

        $pvz_json = [];
        foreach ($pvz_list as $delivery_id => $delivery_pvz) {
        ...
        $this->view->assign([
            'pvz_list' => $pvz_list,
            'pvz_json' => preg_replace('#\\\\#', '\\\\\\\\', json_encode($pvz_json, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_HEX_APOS|JSON_HEX_QUOT)),
        ]);
