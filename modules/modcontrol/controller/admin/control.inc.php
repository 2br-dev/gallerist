...
class Control extends Front
{
    ...
    function helperIndex()
    {
        ...
        if (!defined('CANT_UPLOAD_MODULE')) {
            $items[] = new ToolbarButton\Add($this->url->replaceKey(array($this->action_var => 'add')), t('добавить модуль'), array('noajax' => false));
        }
    ...
    function actionAjaxResetSettings()
    {
        ...
        if ($module->exists()) {
            ...
            if (!$access_error) {
                ...
                $system_settings = array('enabled', 'installed', 'site_id', 'deactivated');
