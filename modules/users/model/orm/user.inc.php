...
class User extends OrmObject
{
        ...
        $this->getPropertyIterator()->append(array(
            t('Основные'),
                ...
                'e_mail' => new Type\Varchar(array(
                    'maxLength' => '150',
                    'description' => 'E-mail',
                    'checker' => array(array(__CLASS__, 'checkAuthField'), null, 'e_mail'),
                    'meVisible' => false,
                    'trimString' => true,
                )),
                'login' => new Type\Varchar(array(
                    ...
                    'unique' => true,
                    'description' => t('Логин'),
                    'checker' => array(array(__CLASS__, 'checkAuthField'), null, 'login'),
                    'meVisible' => false,
                )),
                ...
                'phone' => new CustomType\VerifiedPhone(array(
                    'maxLength' => '50',
                    'description' => t('Телефон'),
                    'checker' => array(array(__CLASS__, 'checkAuthField'), null, 'phone'),
                    'meVisible' => false,
                ...
                'is_enable_two_factor' => new Type\Integer(array(
        ...
        if ($value != '') {
            ...
            if ($_this->isAuthFieldUniq($field)) {
                if (!$_this->isFieldAvailable($field, $value)) {
                    ...
                    return t('Такой %0 уже занят', [$field_title]).
                        ($_this->link_to_auth_in_error
                            ? t(', <a href="%0" class="inDialog">авторизуйтесь</a>', [$config->getAuthorizationUrl()])
                            : '');
                }
            }
    ...
    private function isFieldAvailable($field, $value)
    {
        return OrmRequest::make()
                ->from($this)
                ->where([$field => $value])
                ->where("id != '#this_id'", array('this_id' => $this['id'] ?? 0))
                ->count() == 0;
    }
