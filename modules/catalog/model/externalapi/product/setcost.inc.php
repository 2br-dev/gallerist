    ...
    */
    protected function process($token, $client_id, $client_secret, $product_id, $cost_id, $value = 0.00, $currency = null)
    {
        ...
        if (!is_numeric($cost_id)){
            ...
            if (!$cost){
                throw new ApiException(t('Цена %0 - не найдена. Проверте название.', array($cost_id)));
            }
            ...
            if (!$cost['id']){
                throw new ApiException(t('Цена не найдена.', array($cost_id)));
            }
        }
        ...
        if (!$currency){
            ...
            $current_currency = \Catalog\Model\Orm\Currency::loadByWhere(array(
                'site_id' => \RS\Site\Manager::getSiteId(),
                'title' => $currency
            ));
