...
class VkTools
{
    ...
    public function loadCategoryForProfile(ExportProfile $profile)
    {
        ...
        if (!$token) {
            $profiles_url = Manager::obj()->getAdminUrl(false, array(), 'export-ctrl');

            return t('Не получен AccessToken в профиле экспорта. Перейдите в раздел <a href="%href" target="_blank">Товары -> Экспорт</a> и нажмите на кнопку "Получить access Token"', array(
                'href' => $profiles_url
            ));
        }
