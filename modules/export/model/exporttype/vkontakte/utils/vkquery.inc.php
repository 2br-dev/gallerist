...
class VkQuery extends BaseModel
{
    ...
    public function query(array $request_params, $method, $profile_id = null)
    {
        ...
        if ($this->log) {
            ...
            $this->log->append(t('--> Запрос на адрес %url с параметрами %params' , array(
                'url' => $url,
        ...
        if ($this->log) {
            $this->log->append(t('<-- Ответ на запрос: %response', array(
                'response' => $response
            )));
        }
        ...
        if (isset($result['error'])) {
            ...
            return $this->addError(t('Ошибка запроса к VK API: %0', array($result['error']['error_msg'])));
        }
