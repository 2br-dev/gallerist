...
class Vkontakte extends AbstractType implements ExchangableInterface
{
        ...
        return parent::_init()
            ->append(array(
                ...
                'group_id' => new Type\Varchar(array(
                    ...
                    'checker' => array('ChkEmpty', t('Укажите идентификатор группы ВК'))
                )),
                'client_id' => new Type\Integer(array(
                    ...
                    'checker' => array('ChkEmpty', t('Укажите идентификатор вашего приложения ВК'))
                )),
                'secret_key' => new Type\Varchar(array(
                    ...
                    'checker' => array('ChkEmpty', t('Укажите защищенный ключ вашего приложения ВК'))
                )),
                'access_token' => new Type\Varchar(array(
                ...
                'log_level' => new Type\Integer(array(
                ...
                'image_resize_width' => new Type\Integer(array(
                    'description' => t('Ширина изображений товара для выгрузки'),
                    'checker' => array('chkMinmax', t('Ширина должна быть в диапазоне от 400 до 2000 px'), 400, 2000),
                    'hint' => t('Используется только, если выгружаются не оригиналы изображения'),
                    'default' => '800'
                )),
                'image_resize_height' => new Type\Integer(array(
                    'description' => t('Высота изображений товара для выгрузки'),
                    'checker' => array('chkMinmax', t('Высота должна быть в диапазоне от 400 до 2000 px'), 400, 2000),
                    'hint' => t('Используется только, если выгружаются не оригиналы изображения'),
                    'default' => '800'
                )),
                'export_amount' => new Type\Integer(array(
    ...
    private function uploadPhoto(ExportProfile $profile, $url, $filepath, $main_photo = false)
    {
        ...
        if ($profile['log_level'] == self::LOG_LEVEL_QUERY) {
            $this->log->append(t('--> POST запрос на URL: %url, отправляем изображение: %filepath', array('url' => $url, 'filepath' => $filepath)));
            $this->log->append(t('<-- Получен ответ: %result', array('result' => $result)));
        }
        ...
        $this->log->append(t('Изображение не загружено на сервер VK. Ошибка: %error', array('error' => $result['error']['error_msg'])));
    ...
    function doExchange()
    {
        ...
        if (!$fetcher->issetBuffer()) {
            ...
            $this->log->clean();
            $this->log->append(t('Выбрано %n товаров для выгрузки. Сформирован буфферный файл.', array('n' => count($ids))));
        }

        $this->log->append(t('Запущен экспорт по API профиля(%id): %title', array(
            'id' => $profile['id'],
            'title' => $profile['title']
        )));
        ...
        $this->log->append(t('Отобрана порция из %n товаров для выгрузки', array(
            'n' => count($list)
        )));
        ...
        if ($list) {
            ...
            foreach($products as $product) {
                $this->log->append(t('<< Начало выгрузки товара(%id): %title', array(
                    'id' => $product['id'],
                    'title' => $product['title']
                )));
                ...
                $this->log->append(t('>> Конец выгрузки товара(%id)', array(
                    'id' => $product['id'],
                )));
            }
        }
        ...
        $this->log->append(t('Завершен цикл выгрузки товаров профиля (%id): %title', array(
            'id' => $profile['id'],
            'title' => $profile['title']
        )));
    ...
    private function addExternalOfferData(ExportProfile $profile, $products)
    {
        ...
        if ($ids) {
            ...
            $products_ext_data = array();
            foreach($rows as $row) {
                $row['ext_data_array'] = @json_decode($row['ext_data'], true) ?: array();
        ...
        foreach(clone $product['offers']['items'] as $offer_index => $offer) {
            ...
            if (!$profile['only_available'] || $product->getNum($offer_index) > 0) {
                    ...
                    || (!isset($offer['external_data']) || $offer['external_data']['has_changed'] == 1))
                {
                    ...
                    $this->log->append(t('Комплектация (ID:%id) не нуждается в обновлении', array('id' => $offer['id'])));
                }
            } else {
                $this->log->append(t('Комплктации (ID:%id) нет в наличии. Не проходит по фильтру.', array('id' => $offer['id'])));
            }
     ...
     * @throws \RS\Orm\Exception
     */
    private function getOfferRequestParams(ExportProfile $profile, Product $product, $offer_index, $offer, &$ext_data = array())
    {
    ...
    private function exportOneProductOffer(ExportProfile $profile, Product $product, $offer_index)
    {
        ...
        if (!$request_params['category_id']) {
            $this->log->append(t('Основная категория товара `%title` не сопоставлена с категорией Вконтакте. Экспорт товара невозможен.', array(
        ...
        if (!isset($offer['external_data']['hash']) || $offer['external_data']['hash'] != $hash) {
            if (isset($offer['external_data']['ext_id'])) {
                ...
                if ($result === self::EDIT_ERROR_DELETED_BY_VK) {
                    ...
                    $this->log->append(t('Комплектация успешно обновлена (ID:%id)', array('id' => $offer['id'])));
                }
                ...
                if ($vk_id) {
                    $this->log->append(t('Комплектация успешно создана в виде товара. Присвоен ID: %id', array(
                        'id' => $vk_id
                    )));
                } else {
    ...
    private function uploadPhotos(ExportProfile $profile, Product $product, $offer_index, $is_main_photo, &$ext_data)
    {
        ...
        foreach($product->getImages() as $image) {
            ...
            if ($is_main_photo || $count > 0) {
                ...
                $this->log->append(t('< Начало загрузки %type изображения (%id) URL:%url', array('id' => $image['id'], 'url' => $image_url, 'type' => $is_main_photo ? t('главного') : '')));
                ...
                if (isset($offer['external_data']['ext_data_array']['photos'][(int)$is_main_photo][$image_filename])) {
                    ...
                    $this->log->append(t('Изображение уже загружалось ранее, ID VK:%id', array('id' => $id)));
                } else {
                    ...
                    if ($uri) {
                        ...
                        if ($response && isset($response[0]['id'])) {
                            ...
                            $this->log->append(t('Изображение успешно загружено, ID VK: %id', array(
                                'id' => $response[0]['id']
                            )));
                        } else {
                ...
                $this->log->append(t('> Конец загрузки изображения %id', array('id' => $image['id'])));
            }
            $count++;
        ...
        if (!$image_ids) {
            return $is_main_photo ? null : array();
        }
