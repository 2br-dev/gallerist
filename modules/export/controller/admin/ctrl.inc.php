...
class Ctrl extends \RS\Controller\Admin\Crud
{
    ...
    function helperIndex()
    {
        ...
        $helper->setTable(new Table\Element(array(
            'Columns' => array(
                ...
                new TableType\Yesno('is_enabled', t('Включен'), array('Sortable' => SORTABLE_BOTH, 'toggleUrl' => $this->router->getAdminPattern('ajaxToggleEnabled', array(':id' => '@id')))),
    ...
    function actionShowLog()
    {
        ...
        $helper->setBottomToolbar(new Toolbar\Element(array(
            'Items' => array(
                ...
                new ToolbarButton\Button($this->router->getAdminUrl('showLog', ['refresh' => 1, 'profile_id' => $profile_id]), t('обновить'), array(
                    'attr' => array(
                        'class' => 'btn btn-warning call-update no-update-hash',
                        'data-update-container' => '#log-zone'
                    )
                )),
                new ToolbarButton\Delete($this->router->getAdminUrl('clearLog', ['profile_id' => $profile_id]), t('Очистить'), array(
                    'attr' => array(
    ...
    function actionAjaxDoMarkToExport()
    {
        ...
        return $this->result->addMessage(t('Отмечено для выгрузки %n товаров и комплектаций', array('n' => $count)));
    }
