...
class ExternalRequest
{
    ...
    protected function logRequest()
    {
        if ($this->isEnabledLog()) {
            ...
            if ($this->getPreparedHeaders()) {
                $text .= t('Заголовки запроса') . ' - ' . var_export($this->getPreparedHeaders(), true) . "\n";
            }
            if ($this->getParams()) {
                $text .= t('Параметры запроса') . ' - ' . var_export($this->getParams(), true) . "\n";
            }

            $this->getLog()->append($text);
        }
    }
    ...
    protected function getLog()
    {
        ...
        $id = $this->getSourceId();
        if (!isset($log[$id])) {
            $filename = \Setup::$PATH . \Setup::$LOGS_DIR . "/externalrequests/$id.log";
            $log[$id] = Log::file($filename)->enableDate()->setMaxLength(1048576);
        }
        return $log[$id];
    }
     ...
     * @return bool
     */
    public function isEnabledLog(): bool
    {
