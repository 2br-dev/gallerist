...
class Profile extends AuthorizedFront
{
    ...
    public $use_post_keys = [
        ...
        'openpass', 'current_pass', 'openpass_confirm', 'captcha', 'data', 'changepass', 'birthday', 'photo', 'region',
        'is_author', 'city', 'about'
    ];
    ...
    public function actionIndex()
    {
        ...
        if ($this->isMyPost() && $this->url->checkCsrf()) {
            ...
            if ($user->save($user['id'])) {
                if($user->isModified('region') || $user->isModified('city')){
                    $user->changeLocationAllArts();
                }
...
//                $this->refreshPage();
            ...
            $this->result->addSection('errors', $user->getErrorsByForm());
