 ...
 * --\--
 */
class Picture extends OrmObject
{
    const MAX_RATING = 5;
    ...
    function _init()
    {
        parent::_init()->append([
            ...
            'title' => new Type\Varchar([
                'maxLength' => '255',
                'description' => t('Название картины'),
                'Checker' => ['chkEmpty', t('Укажите название картины')],
                'attr' => [[
...
//                'Checker' => ['chkEmpty', t('Укажите хотя бы одну категорию')],
    ...
    {
        $this['shape'] = @unserialize($this['_shape']);
        $this['orientation'] = @unserialize($this['_orientation']);
        $this['style'] = @unserialize($this['_style']);
        $this['material'] = @unserialize($this['_material']);
        $this['base'] = @unserialize($this['_base']);
        $this['theme'] = @unserialize($this['_theme']);
    ...
    {
        $row = '_'.$product_row;
        $row_value = @unserialize($this[$row]);
    ...
    /**
     * Добавляет значение характеристики
     *
     * @param string $city - значение характеристики
     * @param integer $prop_city_id - id города
     *
     * @return int
     */
    public static function insertPropertyItem($prop_value, $prop_id) {
        $prop = new \Catalog\Model\Orm\Property\Item($prop_id);
        $values = $prop->getAllowedValues();
        $found = false;
        foreach ($values as $id=>$value){
            if (mb_strtolower($value) ==  mb_strtolower(trim($prop_value))){
                $found = $id;
            }
        }
        if (!$found) {
            ...
            $item['alias'] = \RS\Helper\Transliteration::str2url($prop_value);
    ...
    /**
     * Проверка - было ли дабавлено изображение Картины перед сохранением
     * @param $_this
     * @param $value
     * @param $error_text
     * @return bool
     */
    public static function checkSelectedImage($_this, $value, $error_text)
    {
        /**
         * @var \Gallerist\Model\Orm\Picture $_this
         */
    ...
    function beforeWrite($flag)
    {
        ...
        $dir = $url->request('dir', TYPE_INTEGER, 0);
        if ($this['id'] < 0) {
            $this['_tmpid'] = $this['id'];
            unset($this['id']);
        }
        ...
        if($this->isModified('shape')){
            $this['_shape'] = serialize($this['shape']);
        }
        if($this['shape']){
            $this->addProperty($config['prop_shape'], $this['shape']);
        }


        if($this->isModified('orientation')){
            $this['_orientation'] = serialize($this['orientation']);
        }
        if($this['orientation']) {
            $this->addProperty($config['prop_orientation'], $this['orientation']);
        }

        if($this->isModified('style')){
            $this['_style'] = serialize($this['style']);
        }
        if($this['style']) {
            $this->addProperty($config['prop_style'], $this['style']);
        }

        if($this->isModified('material')){
            $this['_material'] = serialize($this['material']);
        }
        if($this['material']) {
            $this->addProperty($config['prop_material'], $this['material']);
        }

        if($this->isModified('base')){
            $this['_base'] = serialize($this['base']);
        }
        if($this['base']) {
            $this->addProperty($config['prop_base'], $this['base']);
        }

        if($this->isModified('theme')){
            $this['_theme'] = serialize($this['theme']);
        }
        if($this['theme']) {
            $this->addProperty($config['prop_theme'], $this['theme']);
        }
        if($this['width']) {
            $this->addProperty($config['prop_width'], $this['width']);
        }
        if($this['height']) {
            $this->addProperty($config['prop_height'], $this['height']);
        }
        $this->addProperty($config['prop_in_stock'], $this['in_stock']);
        $this->addProperty($config['prop_is_original'], $this['is_original']);
        $this->addProperty($config['prop_in_baguette'], $this['in_baguette']);

        $this['city'] = self::insertPropertyItem($owner['city'], $config['prop_city']);
        $this->addProperty($config['prop_city'], $this['city']);

        $this['region'] = self::insertPropertyItem($owner['region'], $config['prop_region']);
        $this->addProperty($config['prop_region'], $this['region']);
