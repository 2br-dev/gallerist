...
class UploadApi extends BaseModel
{
    ...
    function uploadImage($file)
    {
        ...
        if (!in_array($file['type'], array('image/jpeg', 'image/jpg', 'image/pjpeg', 'image/png', 'image/gif'))){
        ...
        if (!$images){
            foreach (glob(\Setup::$ROOT.\Setup::$MODULE_FOLDER."/designer".\Setup::$MODULE_TPL_FOLDER."/img/iconsset/*", GLOB_ONLYDIR) as $folder){
                ...
                foreach (glob($folder."/*") as $file){
                    $title = str_replace(array(".svg", "_"), array("", " "), basename($file));
    ...
    function getListFromFilesFolder($page = 1)
    {
        ...
        foreach($files as $file){
            ...
            if (in_array(mb_strtolower(pathinfo($file, PATHINFO_EXTENSION)), array('jpg','gif','webp','bmp', 'png'))){ //Если это картинка, то вернем к ней размера
