...
class BlocksApi extends BaseModel
{
     ...
     * @return array
     */
    private function getStructureAttrs($class, $change_params = array())
    {
    ...
    function getStructuresJSON()
    {
        ...
        return [
            ...
            'column' => [ //Колонка
                ...
                'type' => "column",
                'additional_class' => '',
                'attrs' => $this->getStructureAttrs('d-col-md-12', array(
                ...
                if (!empty($settings['row']['childs'])){
                    foreach ($settings['row']['childs'] as &$col){
                        ...
                        if ($col['id'] == $col_id){
                            ...
                            if (!empty($col['childs'])){
                                array_splice($col['childs'], $index, 0, array($atomdata));
                            }else{
                                $col['childs'][] = $atomdata;
                            }
                        }
    ...
    function addColumn($block_id, $index, $coldata)
    {
        if ($block = $this->getBlockById($block_id)){
            ...
            if (!empty($settings)){
                if (!empty($settings['row']['childs'])){
                    array_splice($settings['row']['childs'], $index, 0, array($coldata));
                }else{
                    ...
                    foreach ($settings['row']['childs'] as &$col){
                        ...
                        if ($col['id'] == $col_id){
                            ...
                            if (!empty($col['childs'])){
                                ...
                                array_splice($childs, $new_position, 0, array($atomdata));
                                $col['childs'] = array();
