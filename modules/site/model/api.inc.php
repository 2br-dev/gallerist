...
class Api extends EntityList
{
    ...
    {
        parent::__construct(new Orm\Site, array(
            'sortField' => 'sortn',
            'defaultOrder' => 'sortn',
            'nameField' => 'title'
        ));
    ...
    {
        self::copyRights($site_to, $site_from, new AccessMenu(), array(
     ...
     * @return void
     */
    protected function copyRights(Site $site_to, Site $site_from, AbstractObject $orm, $where = array())
    {
        OrmRequest::make()
            ->delete()
            ->from($orm)
            ->where(array_merge(array('site_id' => $site_to['id']), $where))
            ->exec();

        $rights = OrmRequest::make()
            ->from($orm)
            ->where(array_merge(array('site_id' => $site_from['id']), $where))
            ->exec()->fetchAll();
