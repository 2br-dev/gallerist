...
class TelphinCtrl extends Front
{
    ...
    public function actionCheckAuth()
    {
        ...
        $access_token = $provider_object->getAccessToken(array(
        ...
        if ($access_token === false) {
            ...
            return $this->result->addMessage(t('Авторизация прошла успешно, получен токен: %token', array(
                'token' => $access_token
            )));
        }
    }
    ...
    public function actionSetEventUrl()
    {
        ...
        if (!$result) {
            ...
            return $this->result->setSuccess(true)->addMessage(t('Успешно обновлены адреса добавочных: %ids', array(
                'ids' => implode(', ', $result)
            )));
        }
    }
