...
class Tools extends Front
{
    ...
    public function actionTestTelephony()
    {
        ...
        $form_object =  new FormObject(new PropertyIterator(array(
            'provider' => new Type\Varchar(array(
                ...
                'list' => array(array('Crm\Model\Telephony\Manager', 'getProvidersTitles'), Manager::FILTER_ONLY_WITH_TEST)
            )),
            'provider_fields' => new Type\ArrayList(array(
                'description' => '',
    ...
    /**
     * Отображает диалог с логами запросов к/из телефонии
     */
    public function actionLogTelephony()
    {
        if ($error = Rights::CheckRightError('crm', ModuleRights::CALL_HISTORY_OTHER_READ)) {
            return $this->result->setSuccess(false)->addEMessage($error)->addSection('close_dialog', true);
        }
        ...
        $log_content = Manager::getLogContent();
        ...
        $helper = new CrudCollection($this);
        $helper->viewAsForm();
        $helper->setTopTitle(t('Лог обмена данными с провайдерами телефонии'));
        $helper->setBottomToolbar(new Toolbar\Element(array(
            'Items' => array(
                new ToolbarButton\Cancel($this->url->getSavedUrl($this->controller_name . 'index'), t('закрыть')),
                new ToolbarButton\Button($this->router->getAdminUrl('logTelephony', ['refresh' => 1]), t('обновить'), array(
                    'attr' => array(
                        'class' => 'btn btn-warning call-update no-update-hash',
                        'data-update-container' => '#log-zone'
                    )
                )),
                new ToolbarButton\Delete($this->router->getAdminUrl('clearLogTelephony'), t('Очистить'), array(
                    'attr' => array(
        ...
        if ($refresh) {
            ...
            $helper->setForm($this->view->fetch('telephony/show_log.tpl'));
            return $this->result->setTemplate( $helper->getTemplate() );
        }
    }
     ...
     * @return \RS\Controller\Result\Standard
     */
    public function actionClearLogTelephony()
    {
        Manager::clearLog();
    ...
    {
        $form_object = new FormObject(new PropertyIterator(array(
            'delete_all' => new Type\Integer(array(
                'description' => t('Удалить все локальные файлы записей'),
                'checkboxView' => array(1,0)
            )),
            'delete_before_date' => new Type\Datetime(array(
        ...
        $helper->setBottomToolbar(new Toolbar\Element(array(
            'Items' => array(
                new ToolbarButton\SaveForm(null, t('Удалить'), array(
                    'attr' => array(
                        'class' => 'btn-danger'
                    ),
                )),
