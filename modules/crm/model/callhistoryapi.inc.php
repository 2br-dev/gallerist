...
class CallHistoryApi extends EntityList
{
    ...
    {
        parent::__construct(new Orm\Telephony\CallHistory(), array(
            'loadOnDelete' => true
        ));
    }
    ...
    public static function getDataForInteraction($call_history_id)
    {
        ...
        if ($call = $call_api->getOneItem($call_history_id)) {
            $call_data['links'] = array(
                LinkTypeCall::getId() => array($call['id'])
            );

            $client = $call->getOtherUser();
            if ($client['id'] > 0) {
                $call_data['links'][LinkTypeUser::getId()] = array($client['id']);
            }

            if ($call['call_flow'] == CallHistory::CALL_FLOW_IN) {
                $call_data['title'] = t('Входящий звонок от %number', array(
                    'number' => $call['caller_number']
                ));
            } else {
                $call_data['title'] = t('Исходящий звонок от %number', array(
                    'number' => $call['caller_number']
                ));
            }
        }
    ...
    public static function getDataForDeal($call_history_id)
    {
        ...
        if ($call = $call_api->getOneItem($call_history_id)) {
            ...
            if ($client['id'] > 0) {
                ...
                $call_data['client_name'] = t('Клиент с номером %number', array(
                    'number' => $client['phone']
                ));
            }
        }
