...
class LinkTypeCall extends AbstractType
{
    ...
    public function getTabForm()
    {
        $form = new FormObject(new PropertyIterator(array(
            'call_history_id' => new SelectCall(array(
                'description' => t('Поиск по звонкам'),
                'checker' => array('ChkEmpty', t('Звонок не выбран')),
            ))
        )));
    ...
    public function getLinkText()
    {
        if ($this->linked_object['id']) {
            ...
            return t('Звонок не найден (ID: %id)', array(
    ...
    {
        if ($this->linked_object['id']) {
            $url = Manager::obj()->getAdminUrl('edit', array('id' => $this->linked_object->id), 'crm-callhistoryctrl');
            return $url;
        }
    }
}