...
class CallHistory extends OrmObject
{
    ...
    protected static $icon_map = array(
    ...
    function _init()
    {
        parent::_init()->append(array(
            ...
            'call_id' => new Type\Varchar(array(
                'description' => t('Внутренний ID вызова'),
                'visible' => false,
            )),
            'call_api_id' => new Type\Varchar(array(
            ...
            'caller_id' => new Type\Varchar(array(
                'description' => t('ID вызывающего абонента'),
                'visible' => false,
            )),
            'called_number' => new Type\Varchar(array(
            ...
            'called_public_number' => new Type\Varchar(array(
                'description' => t('Публичный номер на который звонит абонент')
            )),
            'event_time' => new Type\DateTime(array(
                'description' => t('Дата и время звонка'),
                'visible' => false,
            )),
            'duration' => new Type\Bigint(array(
                'description' => t('Время разговора, в микросекундах'),
                'visible' => false,
            )),
            'record_id' => new Type\Varchar(array(
                'description' => t('ID файла записи разговора'),
                'allowEmpty' => false,
                'visible' => false
            )),
            'call_status' => new Type\Varchar(array(
                'description' => t('Статус звонка'),
                'listFromArray' => array(self::getCallStatuses())
            )),
            'call_sub_status' => new Type\Varchar(array(
                'description' => t('Результат звонка'),
                'listFromArray' => array(self::getCallSubStatuses())
            )),
            'call_flow' => new Type\Enum(array_keys(self::getCallFlows()), array(
                'description' => t('Направление вызова'),
                'listFromArray' => array(self::getCallFlows())
            )),
            'custom_data' => new Type\ArrayList(array(
                'description' => t('Произвольные данные'),
                'visible' => false,
            )),
            '_custom_data' => new Type\Text(array(
                'description' => t('Произвольные данные'),
                'visible' => false
            )),
            'send_to_browser' => new Type\Integer(array(
                'runtime' => true,
        ...
        $this->addIndex(array('call_id'), self::INDEX_UNIQUE);
        $this->addIndex(array('caller_number'), self::INDEX_KEY);
        $this->addIndex(array('called_number'), self::INDEX_KEY);
        $this->addIndex(array('record_id'), self::INDEX_KEY);
    }
    ...
    {
        if (in_array($this['call_status'], array(self::CALL_STATUS_CALLING, self::CALL_STATUS_ANSWER))) {
            return true;
        }
    ...
    {
        if ($this['record_id']) {
            return \RS\Router\Manager::obj()->getAdminUrl(false, array(
                'do' => 'getRecord',
