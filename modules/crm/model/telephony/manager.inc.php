...
class Manager
{
    ...
    public static function getProviders($cache = true)
    {
        ...
        if (!$cache || $result === null) {
            $result = array();
            $event_result = EventManager::fire('crm.telephony.getproviders', array());
     ...
     * @throws \RS\Event\Exception
     */
    public static function getProvidersTitles($filter = self::FILTER_NO, $first = array(), $cache = true)
    {
    ...
    {
        return \Setup::$PATH.\Setup::$LOGS_DIR.'/telephony_in_events.log';
    }
    ...
    {
        $log_filepath = self::getLogFilepath();
        if (file_exists($log_filepath)) {
            return file_get_contents($log_filepath);
        }
    }
