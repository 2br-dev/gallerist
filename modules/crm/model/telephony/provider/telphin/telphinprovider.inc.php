...
class TelphinProvider extends AbstractProvider
{
    ...
    public function getActionsByCall(CallHistory $call)
    {
        ...
        if ($this->canApiRequest()) {
            ...
            $hangup_url = $router->getAdminUrl('doAction', array(
     ...
     */
    public function getAccessToken($params = array(), $force = false)
    {
        ...
        if ($result->getStatusCode() == 200) {
            ...
            if ($json && !empty($json->access_token)) {
                ...
                $this->setError(t('Не удалось распарсить резульатат запроса %raw', array(
                    'raw' => $result->getRawData()
                )));
            }
        } else {
            $json = $result->getJsonData();
            $this->setError(t('Неудачная авторизация. Статус ответа %status. Ответ: %response.', array(
    ...
    public function getRecordData(CallHistory $call, $find_local = true)
    {
        if ($call['record_id']) {
            ...
            if ($response->getStatusCode() != 200) {
                return $this->setError(t('Не удалось выполнить запрос. Ошибка: %code', array(
    ...
    public function getClientId($cache = true)
    {
        ...
        if ($cache) {
            return \RS\Cache\Manager::obj()
                ->expire(0)
                ->request(array($this, 'getClientId'), false, md5($config['telphin_app_id'], $config['telphin_app_secret']));
        } else {
            $requester = $this->getRequester();
            ...
            return $this->setError(t('Не удалось получить client_id. Статус: %status', array(
    ...
    public function getExtensionIdByName($extension_name, $cache = true)
    {
        ...
        if ($cache) {
            return \RS\Cache\Manager::obj()
                ->expire(0)
                ->request(array($this, 'getExtensionIdByName'), $extension_name, false, $uniq);
        } else {
            $requester = $this->getRequester();
    ...
    public function CallPhoneNumber($number)
    {
        ...
        if ($extension_name) {
            ...
            if ($extension_id) {
                ...
                $requester->setData(json_encode(array(
                    'src_num' => array($extension_name),
    ...
    public function setEventUrl($telphin_app_id,
                                ...
                                $telphin_user_map)
    {
        $access_token = $this->getAccessToken(array(
        ...
        foreach($extension_ids as $id) {
            ...
            if ($result->getStatusCode() == 200) {
                foreach($result->getJsonData() as $item) {
                    ...
                    $mask = rtrim(str_replace(array('http://', 'https://', $this->getUrlSecret()), '', $full_url), '/');
                    ...
                    if (stripos($item->url, $mask) !== false) {
                        ...
                        if ($subresult->getStatusCode() != 204) {
                            return $this->setError(t('Не удалось удалить EventUrl ID: %id', array(
                                'id' => $item->id
                            )));
                        }
                    }
                ...
                return $this->setError(t('Не удалось выполнить запрос. Статус: %status', array(
                    'status' => $result->getStatusCode()
                )));
            }
        }
    ...
    private function addEventUrls($extension_ids, $sip_map)
    {
        ...
        foreach($extension_ids as $id) {
            ...
            foreach($this->getAllowEventTypes() as $event_type => $method) {
                ...
                $response = $requester->request($url);
                if ($response->getStatusCode() != 201) {
                    return $this->setError(t('Не удалось выполнить запрос на установку EventUrl. Статус: %status', array(
                        'status' => $response->getStatusCode()
                    )));
                }
            }
