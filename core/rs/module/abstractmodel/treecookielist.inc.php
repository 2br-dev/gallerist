...
abstract class TreeCookieList extends TreeList
{
    ...
    public function __construct(AbstractObject $orm_element, array $options = array())
    {
    ...
    public function getOpenedElements()
    {
        if ($this->opened_elements === null) {
            $this->opened_elements = isset($_COOKIE[$this->uniq]) ? explode(',', $_COOKIE[$this->uniq]) : array();
        }
