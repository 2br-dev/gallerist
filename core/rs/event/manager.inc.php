                ...
                if (is_callable($callback)) {
                    ...
                    if ($new_params !== null) {
                        ...
                        if (gettype($params) != $params_type) {
                            self::throwException(t("Обработчик %0::%1. события %2 должен вернуть значение того же типа, что и 'params' (%3) или NULL", array($callback[0], $callback[1], $event, $params_type)));
                        }
                    }
                    if ($this_event->isStopped()) break;
                } else {
                    self::throwException(t("Не найден обработчик '%0' события '%1'", array(implode('::', $callback), $event)));
                }
            }
        }
