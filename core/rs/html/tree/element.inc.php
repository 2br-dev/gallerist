...
class Element extends AbstractHtml
{
    ...
    public function getData($with_root = true)
    {
        if ($with_root && !empty($this->options['rootItem'])) {
            return array_merge(array(new TreeListFakeNode($this->options['rootItem'])), $this->data->getItems());
        }
        return $this->data;
    }
    ...
    private function findActivePath($data, $current_path = array())
    {
        foreach ($data as $item) {
            if ($item['fields'][$this->options['activeField']] == $this->options['activeValue']) {
                return array_merge($current_path, array($item->getObject()));
            }
            if (count($item->getChilds())) {
                if ($result = $this->findActivePath($item->getChilds(), array_merge($current_path, array($item->getObject())))) {
                    return $result;
                }
            }
        }
