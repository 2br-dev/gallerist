...
<div class="nav-actions">
    {if $is_auth}
        <ul class="profile-menu">
            ...
            <li>
                <ul class="profile-menu-inner">
                    <li><p class="name-account">{$current_user.name} {$current_user.surname}</p></li>
    ...
    {else}
        <a href="{$authorization_url}" class="btn btn-small rs-in-dialog">
            <span class="white-text">Войти</span>
        </a>
    {/if}
    <div class="favorite-header rs-favorite-block favs{if $countFavorite} active{/if}" data-favorite-url="{$router->getUrl('catalog-front-favorite')}">
        {$countFavorite = \Catalog\Model\FavoriteApi::getInstance()->getFavoriteCount()}
        <a href="/favorites.html">
            <i class="mdi mdi-heart"></i>
            <span class="link-chip rs-favorite-items-count count">{$countFavorite}</span>
        </a>
    </div>

    <a class="search-trigger"><i class="mdi mdi-magnify"></i></a>
</div>
