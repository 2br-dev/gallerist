...
<ul class="topCategory" id="topCategory">
    ...
    <li class="item_{$dir@iteration}{if $dir->getChildsCount()} node{/if}" {$dir.fields->getDebugAttributes()}><a class="{if $dir->getChildsCount()}dirChild{/if}" href="{$dir.fields->getUrl()}">{$dir.fields.name}</a>
